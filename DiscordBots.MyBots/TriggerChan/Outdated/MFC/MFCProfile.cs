	...
	public class MFCProfile {
		...
		public static async Task<MFCProfile> LoadProfile(string username) {
			...
			using (var client = new HttpClient()) {
				string raw = await (await client.GetAsync($"https://myfigurecollection.net/profile/{username}").ConfigureAwait(false)).Content.ReadAsStringAsync().ConfigureAwait(false);
		...
		public static MFCCategory ParseCategory(MFCCategoryType type, HtmlNode section) {
			MFCCategory cat = new MFCCategory { Type = type };
