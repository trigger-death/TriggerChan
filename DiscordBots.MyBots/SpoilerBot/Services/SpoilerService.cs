...
namespace TriggersTools.DiscordBots.SpoilerBot.Services {
		...
		where TDbContext : DbContextEx, ISpoilerDbContext
	{
		...
		/// </summary>
		public SpoilerService(DiscordBotServiceContainer services,
		...
		private async Task OnDeletingEndUserData(EndUserDataEventArgs e) {
			if (e.Type == EndUserDataType.User) {
				using (var db = GetDb<TDbContext>()) {
					IDbSpoilerUser user = await db.FindSpoilerUserAsync(e.Id).ConfigureAwait(false);
					if (user == null)
						return;
		...
		public async Task RenameSpoilerAsync(TDbContext db, Spoiler spoiler, string newTitle) {
			try {
				if (Client.GetChannel(spoiler.ChannelId) is ITextChannel channel &&
					await channel.GetMessageAsync(spoiler.MessageId).ConfigureAwait(false) is IUserMessage message)
				{
		...
		Task ISpoilerService.RenameSpoilerAsync(DbContextEx db, Spoiler spoiler, string newTitle) {
			return RenameSpoilerAsync((TDbContext) db, spoiler, newTitle);
		}
		...
		/// <param name="messageId">The message Id of the spoiler.</param>
		/// <returns>The found spoiler, or null.</returns>
		public Task<Spoiler> FindSpoilerAsync(TDbContext db, ulong messageId) {
			return db.Spoilers.FindAsync(messageId);
		}
		Task<Spoiler> ISpoilerService.FindSpoilerAsync(DbContextEx db, ulong messageId) {
			return FindSpoilerAsync((TDbContext) db, messageId);
		}
		/// <summary>
		...
		/// <param name="userId">The user Id of the spoiled user.</param>
		/// <returns>The found spoiled user, or null.</returns>
		public Task<SpoiledUser> FindSpoiledUserAsync(TDbContext db, ulong messageId, ulong userId) {
			return db.SpoiledUsers.FindAsync(messageId, userId);
		}
		Task<SpoiledUser> ISpoilerService.FindSpoiledUserAsync(DbContextEx db, ulong messageId, ulong userId) {
			return FindSpoiledUserAsync((TDbContext) db, messageId, userId);
		}
		/// <summary>
		...
		/// <param name="authorId">The Id of the author who posted the spoiler.</param>
		/// <param name="channelId">The channel the spoiler was posted in.</param>
		public async Task<Spoiler> FindLastSpoilerByUserAsync(TDbContext db, ulong authorId, ulong channelId) {
			IDbSpoilerUser user = await db.FindSpoilerUserAsync(authorId).ConfigureAwait(false);
			if (user == null)
				return null;
		...
		Task<Spoiler> ISpoilerService.FindLastSpoilerByUserAsync(DbContextEx db, ulong authorId, ulong channelId) {
			return FindLastSpoilerByUserAsync((TDbContext) db, authorId, channelId);
		}
		/// <summary>
		...
		private async Task RemoveSpoilerAsync(TDbContext db, Spoiler spoiler) {
		...
		private Embed BuildEmbed(Spoiler spoiler, IEmbed oldEmbed = null, bool deactivated = false) {
			...
			EmbedBuilder embed = new EmbedBuilder {
				...
				Title = FormatTitle(spoiler, deactivated),
			};
			if (oldEmbed != null) {
		...
		private async Task PostSpoilerContentAsync(IUser user, Spoiler spoiler, SpoiledUser spoiledUser) {
			...
			EmbedBuilder embed = new EmbedBuilder {
				...
				Color = configParser.EmbedColor,
				Timestamp = spoiler.TimeStamp.ToLocalTime(),
			};
			if (author != null)
