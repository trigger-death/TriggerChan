...
namespace TriggersTools.DiscordBots.TriggerChan.Util {
	...
	public static class StringExtensions {
		...
		/*public static int IndexOfUnescaped(this string str, char value) {
			return IndexOfUnescaped(str, value, 0);
		}

		public static int IndexOfUnescaped(this string str, char value, int startIndex) {
			int index = str.IndexOf(value, startIndex);
			while (index != -1 && str.IsEscaped(index)) {
				index = str.IndexOf(value, index + 1);
			}
			return index;
		}

		public static int IndexOfUnescaped(this string str, string value) {
			return IndexOfUnescaped(str, value, 0);
		}

		public static int IndexOfUnescaped(this string str, string value, int startIndex) {
			int index = str.IndexOf(value, startIndex);
			while (index != -1 && str.IsEscaped(index)) {
				index = str.IndexOf(value, index + 1);
			}
		...
		public static int IndexOfUnescaped(this string str, IEnumerable<CodeBlock> eblocks,
			char value)
		{
			return IndexOfUnescaped(str, eblocks, value.ToString(), 0);
		}
		
		public static int IndexOfUnescaped(this string str, IEnumerable<CodeBlock> eblocks,
			char value, int startIndex)
		{
			return IndexOfUnescaped(str, eblocks, value.ToString(), startIndex);
		}
		
		public static int IndexOfUnescaped(this string str, IEnumerable<CodeBlock> eblocks,
			string value)
		{
			return IndexOfUnescaped(str, eblocks, value, 0);
		}

		public static int IndexOfUnescaped(this string str, IEnumerable<CodeBlock> eblocks,
			string value, int startIndex)
		{
			List<CodeBlock> blocks = eblocks as List<CodeBlock> ?? eblocks.ToList();
