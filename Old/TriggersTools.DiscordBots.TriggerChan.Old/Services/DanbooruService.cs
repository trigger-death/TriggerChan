...
namespace TriggersTools.DiscordBots.TriggerChan.Services {
	public class DanbooruService : BotServiceBase {
		...
		public async Task<int> ScoreReactions(IUserMessage message) {
			int agrees = (await message.GetReactionUsersAsync(BotReactions.Agreeable, 100).FirstOrDefault())?.Count ?? 0;
			int disagrees = (await message.GetReactionUsersAsync(BotReactions.Dangerous, 100).FirstOrDefault())?.Count ?? 0;
