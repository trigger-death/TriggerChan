...
namespace Victoria
{
    public sealed class LavaNode : IDisposable
    {
        ...
        internal async Task StartAsync()
        {
            ...
            await LavaSocket.ConnectAsync().ConfigureAwait(false);
		}
        ...
        public async Task StopAsync()
        {
            foreach (var connection in _players)
                await connection.Value.DisconnectAsync().ConfigureAwait(false);
        ...
        private async Task OnClose()
        {
            ...
            await Task.Delay(_retryInterval).ContinueWith(_ => LavaSocket.ConnectAsync()).ConfigureAwait(false);
		}
