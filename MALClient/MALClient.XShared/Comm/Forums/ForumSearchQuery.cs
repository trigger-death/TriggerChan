	...
	public static class ForumSearchQuery {
			...
			if (query.Length > 2)
				try {
					...
					using (var client = new HttpClient()) {
						resp = await client.GetAsync($"/forum/search?q={query}&u=&uloc=1&loc={scope}");
					}
		...
		public static async Task<ForumBoardContent> GetRecentTopics(string user) {
			...
			try {
				HttpResponseMessage resp;
				using (var client = new HttpClient()) {
					resp = await client.GetAsync($"/forum/search?u={user}&q=&uloc=1&loc=-1");
				}
		...
		public static async Task<ForumBoardContent> GetWatchedTopics() {
			...
			try {
				HttpResponseMessage resp;
				using (var client = new HttpClient()) {
					resp = await client.GetAsync("/forum/?action=viewstarred");
				}
