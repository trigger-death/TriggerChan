...
namespace TriggersTools.DiscordBots {
	...
	public class DiscordStartup {
		...
		/// <summary>
		/// The argument used to specify a debug operation is running the Discord bot.<para/>
		/// This means the program will wait for input after crashing.
		/// </summary>
		...
		/// <summary>
		/// True if <see cref="DebugArgument"/> was passed, used to wait for input after crashing.
		/// </summary>
		private readonly bool isRunningDebug;
		...
		private DiscordStartup(string[] args, IDiscordBot discordBot, bool allowMultipleInstances) {
			...
			if (args.Length > 0 && args[0] == DebugArgument) {
				isRunningDebug = true;
		...
		public bool SingleInstanceCheck() {
			if (allowMultipleInstances)
				return true;
			if (mutex == null) // "Global\" prefix is required for Ubuntu
				mutex = new Mutex(true, $@"Global\{discordBot.GetDiscordToken()}", out mutexCreatedNew);
			if (!mutexCreatedNew) {
