...
namespace TriggersTools.DiscordBots.TriggerChan.Services {
	...
	/// </summary>
	public partial class SpoilerService : DiscordBotService {
		...
		/// <summary>
		/// Constructs the <see cref="SpoilerService"/>.
		/// </summary>
		public SpoilerService(DiscordBotServiceContainer services) : base(services) {
		...
		private async Task OnDeletingEndUserData(EndUserDataEventArgs e) {
			if (e.Type == EndUserDataType.User) {
				using (var db = GetDb<TriggerDbContext>()) {
					User user = await db.Users.FindAsync(e.Id).ConfigureAwait(false);
					if (user == null)
						return;
		...
		private async Task OnReactionAdded(Cacheable<IUserMessage, ulong> msg, ISocketMessageChannel channel, SocketReaction reaction) {
			...
			if (!user.IsBot && TriggerReactions.ViewSpoiler.Equals(emote)) {
			...
			if (msg.Channel is IDMChannel) {
				...
				if (input == "cancel") {
					...
					try {
						...
						await msg.AddReactionAsync(TriggerReactions.Exception).ConfigureAwait(false);
		...
		private async Task DeactivateSpoilerAsync(Spoiler spoiler) {
			try {
				if (Client.GetChannel(spoiler.ChannelId) is ITextChannel channel &&
					...
					await message.RemoveReactionAsync(TriggerReactions.ViewSpoiler, Client.CurrentUser).ConfigureAwait(false);
				}
			} catch { }
		}
		...
		public async Task RenameSpoilerAsync(TriggerDbContext db, Spoiler spoiler, string newTitle) {
			try {
				if (Client.GetChannel(spoiler.ChannelId) is ITextChannel channel &&
					await channel.GetMessageAsync(spoiler.MessageId).ConfigureAwait(false) is IUserMessage message)
				{
		...
		/// <param name="messageId">The message Id of the spoiler.</param>
		/// <returns>The found spoiler, or null.</returns>
		public Task<Spoiler> FindSpoilerAsync(TriggerDbContext db, ulong messageId) {
		...
		/// <param name="userId">The user Id of the spoiled user.</param>
		/// <returns>The found spoiled user, or null.</returns>
		public Task<SpoiledUser> FindSpoiledUserAsync(TriggerDbContext db, ulong messageId, ulong userId) {
		...
		/// <param name="authorId">The Id of the author who posted the spoiler.</param>
		/// <param name="channelId">The channel the spoiler was posted in.</param>
		public async Task<Spoiler> FindLastSpoilerByUserAsync(TriggerDbContext db, ulong authorId, ulong channelId) {
			User user = db.Users.Find(authorId);
			if (user == null)
				return null;
		...
		private async Task RemoveSpoilerAsync(TriggerDbContext db, Spoiler spoiler) {
		...
		private Embed BuildEmbed(Spoiler spoiler, IEmbed oldEmbed = null, bool deactivated = false) {
			...
			embed.WithColor(ColorUtils.Parse(Config["embed_color"]));
			if (oldEmbed != null) {
		...
		private async Task PostSpoilerAsync(Spoiler spoiler) {
			...
			await message.AddReactionAsync(TriggerReactions.ViewSpoiler).ConfigureAwait(false);
