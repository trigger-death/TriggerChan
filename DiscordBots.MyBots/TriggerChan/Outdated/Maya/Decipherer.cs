...
namespace Maya.Music.Youtube {
	internal static class Decipherer {
		public static string DecipherWithVersion(string cipher, string cipherVersion, WebProxy proxy) {
			...
			string functNamePattern = @"(\w+)&&(\w+)\.set\(\w+,(\w+)\(\1\)\);return\s+\2"; //Regex Formed To Find Word or DollarSign
