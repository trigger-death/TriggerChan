    ...
    internal sealed class LavaSocket : IDisposable
    {
        ...
        public async Task ConnectAsync()
        {
            ...
            try
            {
                await _webSocket.ConnectAsync(new Uri($"ws://{_config.Endpoint.Host}:{_config.Endpoint.Port}"),
                    CancellationToken.None).ContinueWith(AfterConnect).ConfigureAwait(false);
			}
            catch
            {
                // Ignored
        ...
        private void AfterConnect(Task connectTask)
        {
            if (connectTask.IsCanceled || connectTask.IsFaulted || connectTask.Exception != null)
            {
                ...
                Task.Run(async () =>
                {
                    ...
                    await ReceiveDataAsync().ConfigureAwait(false);
        ...
        private async Task ReceiveDataAsync()
        {
            try
            {
                while (true)
                {
                    var data = await ReceiveAsync().ConfigureAwait(false);
					if (data.Length <= 0)
                        continue;
        ...
        private async Task<byte[]> ReceiveAsync()
        {
            using (var stream = new MemoryStream())
            {
                ...
                {
                    var result = await _webSocket.ReceiveAsync(segment, CancellationToken.None).ConfigureAwait(false);
