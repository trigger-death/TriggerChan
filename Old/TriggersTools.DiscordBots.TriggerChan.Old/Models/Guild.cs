...
namespace TriggersTools.DiscordBots.TriggerChan.Models {
	public class Guild : SettingsBase {
		...
		public ulong[] GetPublicRoles() {
			string[] rolesStr = PublicRolesStr.Split(new[] { '|' }, StringSplitOptions.RemoveEmptyEntries);
			ulong[] roles = new ulong[rolesStr.Length];
			for (int i = 0; i < roles.Length; i++) {
				roles[i] = ulong.Parse(rolesStr[i]);
			}
			return roles;
		}
